...
class CaregiverRegistrationActivity : AppCompatActivity() {
    ...
    private val REQUEST_IMAGE_PICK_FRONT_LICENSE = 103
    private val REQUEST_IMAGE_PICK_BACK_LICENSE = 104

    private var frontCardFile: File? = null
    private var backCardFile: File? = null
    private var frontLicenseFile: File? = null
    private var backLicenseFile: File? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.caregiverCardFront.setOnClickListener { checkPermissionsAndSelectImageOption(1) }
        binding.caregiverCardBack.setOnClickListener { checkPermissionsAndSelectImageOption(2) }
        binding.drivingCardFront.setOnClickListener { checkPermissionsAndSelectImageOption(3) }
        binding.drivingCardBack.setOnClickListener { checkPermissionsAndSelectImageOption(4) }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            when (requestCode) {
                ...
                REQUEST_IMAGE_PICK_FRONT_LICENSE -> {
                    frontLicenseFile = data?.let { getFileFromIntent(it) }
                }

                REQUEST_IMAGE_PICK_BACK_LICENSE -> {
                    backLicenseFile = data?.let { getFileFromIntent(it) }
                }

                REQUEST_IMAGE_CAPTURE + 1 -> {
                    val bitmap = data?.extras?.get("data") as Bitmap
                    frontCardFile = convertBitmapToFile(bitmap, "front_card.jpg")
                    binding.caregiverCardFront.text = frontCardFile!!.name.toString()
                }

                REQUEST_IMAGE_CAPTURE + 2 -> {
                    val bitmap = data?.extras?.get("data") as Bitmap
                    backCardFile = convertBitmapToFile(bitmap, "back_card.jpg")
                    binding.caregiverCardBack.text = backCardFile!!.name.toString()
                }

                REQUEST_IMAGE_CAPTURE + 3 -> {
                    val bitmap = data?.extras?.get("data") as Bitmap
                    frontLicenseFile = convertBitmapToFile(bitmap, "front_license.jpg")
                    binding.drivingCardFront.text = frontLicenseFile!!.name.toString()
                }

                REQUEST_IMAGE_CAPTURE + 4 -> {
                    val bitmap = data?.extras?.get("data") as Bitmap
                    backLicenseFile = convertBitmapToFile(bitmap, "back_license.jpg")
                    binding.drivingCardBack.text = backLicenseFile!!.name.toString()
                }
            }
        }
    }
    ...
    private fun registerUser() {
        ...
        if (name.isEmpty()) {
            ...
            return
        } else if (frontCardFile == null || backCardFile == null || frontLicenseFile == null || backLicenseFile == null) {
